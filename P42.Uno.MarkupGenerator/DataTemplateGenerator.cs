...
internal sealed class DataTemplateGenerator : IncrementalExtensionsGeneratorBase<DataTemplateInfo>
{
    private protected override EquatableArray<DataTemplateInfo>? GetInfoForType(INamedTypeSymbol namedType)
    {
        ...
        var GenerationTypeInfo = global::Uno.Extensions.Markup.Generators.GenerationTypeInfo.From(namedType);
      ...
      CancellationToken cancellationToken)
    {
        foreach (var info1 in infos)
        {
            ...
            CreatePropertyBuilder(ref classBuilder, info.GenerationTypeInfo, info.PropertyName)
                .AddParameter("Type", "templateType")
                .WithBody(w =>
                {
                    w.AppendUnindentedLine("#if HAS_UNO");
                    w.AppendLine("var template = new DataTemplate(() => (UIElement)Activator.CreateInstance(templateType));");
                    w.AppendUnindentedLine("#else");
                    w.AppendLine("var template = UIElementExtensions.AsDataTemplate(templateType);");
            ...
            CreatePropertyBuilder(ref classBuilder, info.GenerationTypeInfo, info.PropertyName)
                .AddGeneric("TTemplate", b => b.AddConstraint($"global::{QualifiedTypeName.UIElement}, new()") )
                .WithBody(w =>
                {
                    w.AppendUnindentedLine("#if HAS_UNO");
                    w.AppendLine("var template = new DataTemplate(() => new TTemplate());");
                    w.AppendUnindentedLine("#else");
                    w.AppendLine("var template = UIElementExtensions.AsDataTemplate(typeof(TTemplate));");
                    w.AppendUnindentedLine("#endif");
                    w.AppendLine($"element.{info.PropertyName} = template;");
                    w.AppendLine("return element;");
                });
