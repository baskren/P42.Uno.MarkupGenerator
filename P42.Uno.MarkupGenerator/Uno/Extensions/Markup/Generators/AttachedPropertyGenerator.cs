...
// Type: Uno.Extensions.Markup.Generators.AttachedPropertyGenerator
  ...
  IncrementalExtensionsGeneratorBase<AttachedPropertyInfo>
{
    ...
    private void CreateStylePropertyBuilder(
      ...
      SourceProductionContext context)
    {
        ClassBuilder builder1 = CodeBuilder.Create(typeInfo.TypeContainingNamespace).Nullable(NullableState.Enable).AddClass("CSharpMarkupAttachedPropertyStyleBuilderExtensions").MakeStaticClass().WithAccessModifier((Accessibility)6).DisableWarning("Uno0001").AddNamespaceImport("System").AddNamespaceImport("System.Collections.Generic").AddNamespaceImport("System.Runtime.CompilerServices").AddNamespaceImport("System.Linq").AddNamespaceImport("System.Linq.Expressions").AddNamespaceImport("Microsoft.UI.Xaml").AddNamespaceImport("Microsoft.UI.Xaml.Data").AddNamespaceImport("Uno.Extensions.Markup").AddNamespaceImport(typeInfo.TypeContainingNamespace);
        builder1.AddMethod(typeInfo.TypeName).WithAccessModifier(typeInfo.DeclaredAccessibility).MakeStaticMethod().WithReturnType("StyleBuilder<T>").AddGeneric("T", x => x.AddConstraint("global::Microsoft.UI.Xaml.DependencyObject")).AddParameter("this StyleBuilder<T>", "builder").AddParameter($"Action<{typeInfo.TypeName}PropertyStyleBuilder<T>>", "configureBuilder").WithBody(w =>
        {
        ...
        ClassBuilder builder = CodeBuilder.Create(typeInfo.TypeContainingNamespace).Nullable(NullableState.Enable).AddClass(typeInfo.TypeName + "PropertyStyleBuilder").WithAccessModifier(typeInfo.DeclaredAccessibility).AddGeneric("T", x => x.AddConstraint("global::Microsoft.UI.Xaml.DependencyObject")).DisableWarning("Uno0001").AddNamespaceImport("System").AddNamespaceImport("System.Collections.Generic").AddNamespaceImport("System.Runtime.CompilerServices").AddNamespaceImport("System.Linq").AddNamespaceImport("System.Linq.Expressions").AddNamespaceImport("Microsoft.UI.Xaml").AddNamespaceImport("Microsoft.UI.Xaml.Data").AddNamespaceImport("Uno.Extensions.Markup").AddNamespaceImport(typeInfo.TypeContainingNamespace);
        builder.AddConstructor(new Accessibility?((Accessibility)6)).AddParameter("StyleBuilder<T>", "builder").WithBody(w => w.AppendLine("_builder = builder;"));
        builder.AddProperty("_builder").SetType("StyleBuilder<T>").MakePrivateProperty().WithReadonlyValue();
    ...
    private void CreatePropertyBuilder(
      ...
      SourceProductionContext context)
    {
        ClassBuilder builder = CodeBuilder.Create(typeInfo.TypeContainingNamespace).Nullable(NullableState.Enable).AddClass(typeInfo.TypeName + "PropertyBuilder").WithAccessModifier(typeInfo.DeclaredAccessibility).DisableWarning("Uno0001").AddNamespaceImport("System").AddNamespaceImport("System.Collections.Generic").AddNamespaceImport("System.Runtime.CompilerServices").AddNamespaceImport("System.Linq").AddNamespaceImport("System.Linq.Expressions").AddNamespaceImport("Microsoft.UI.Xaml").AddNamespaceImport("Microsoft.UI.Xaml.Data").AddNamespaceImport("Uno.Extensions.Markup").AddNamespaceImport(typeInfo.TypeContainingNamespace);
        builder.AddProperty("_instance").SetType(builder.Name + "?").WithAccessModifier((Accessibility)1).MakeStatic().WithValue("default", CodeGenHelpers.ValueType.Null);
        builder.AddProperty("_weakElement").SetType("WeakReference<DependencyObject>?").MakePrivateProperty().UseAutoProps();
        builder.AddProperty("_element").SetType("DependencyObject?").MakePrivateProperty().WithGetter(w =>
        {
        ...
        }).WithSetter(w => w.If("value is null").WithBody(x => x.AppendLine("_weakElement = null;")).Else().WithBody(x => x.AppendLine("_weakElement = new WeakReference<DependencyObject>(value);")).EndIf());
        builder.AddConstructor(new Accessibility?((Accessibility)1)).WithBody(w => w.AppendLine("// Intentionally left empty"));
        builder.AddMethod("GetInstance").MakePublicMethod().MakeStaticMethod().WithReturnType(builder.Name).AddParameter("DependencyObject", "element").WithBody(w =>
        {
      ...
      EquatableArray<AttachedPropertyInfo> attachedProperties)
    {
        ...
        foreach (AttachedPropertyInfo attachedProperty in attachedProperties)
        {
            ...
            if (!attachedProperty.PropertyTypeIsNullableValueType)
                typeFullyQualified += "?";
            methodBuilder.AddParameterWithDefaultValue(typeFullyQualified, attachedProperty.Name.Camelcase(), -1);
        }
        methodBuilder.WithBody(w =>
        {
            foreach (AttachedPropertyInfo attachedProperty in attachedProperties)
            {
                ...
                w.If(condition).WithBody(x => x.AppendLine($"element.SetValue({prop.DependencyProperty}, {paramValue});")).EndIf();
            }
            w.AppendLine("return element;");
        });
    }
    ...
    {
        return builder.AddMethod(info.TypeName).MakePublicMethod().MakeStaticMethod().AddAttribute("global::Uno.Extensions.Markup.Internals.MarkupExtensionAttribute").AddGeneric("T", x => x.AddConstraint("DependencyObject")).AddParameter("this T", "element").WithReturnType("T");
    }
    ...
    {
        return builder.AddMethod(prop.Name).MakePublicMethod().WithReturnType(builder.Name);
    }
