...
// Type: Uno.Extensions.Markup.Generators.StyleBuilderGenerator
...
internal sealed class StyleBuilderGenerator : IncrementalExtensionsGeneratorBase<StyleBuilderInfo>
{
      ...
      INamedTypeSymbol namedType)
    {
        ...
        return new EquatableArray<StyleBuilderInfo>?(builder.ToImmutableArray<StyleBuilderInfo>().AsEquatableArray<StyleBuilderInfo>());
    }
    ...
    private static void GenerateControlTemplateExtensions(ClassBuilder builder, StyleBuilderInfo info)
    {
        ...
        MethodBuilder methodBuilder = builder.AddMethod(info.PropertyName).MakePublicMethod().MakeStaticMethod().WithReturnType(returnType).AddGeneric("TRootControl", b => b.AddConstraint("UIElement")).AddParameter<MethodBuilder>("this " + returnType, nameof(builder)).AddParameter<MethodBuilder>($"Func<{styleBuilderTypeParameter}, TRootControl>", delegateName).WithBody(w =>
        {
      ...
      StyleBuilderInfo info)
    {
        ...
        MethodBuilder methodBuilder = builder.AddMethod(info.PropertyName).MakePublicMethod().MakeStaticMethod().WithReturnType(returnType).AddGeneric("T").AddParameter<MethodBuilder>("this " + returnType, nameof(builder)).AddParameter<MethodBuilder>("Func<T, UIElement>", propName).WithBody(w =>
        {
            w.AppendUnindentedLine("#nullable disable");
      ...
      string propertyName)
    {
        ...
        MethodBuilder builder1 = builder.AddMethod(propertyName).MakePublicMethod().MakeStaticMethod().AddAttribute("global::Uno.Extensions.Markup.Internals.MarkupExtensionAttribute").WithReturnType(returnType).AddParameter<MethodBuilder>("this " + returnType, nameof(builder));
        if (!generationTypeInfo.IsSealed)
